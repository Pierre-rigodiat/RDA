...
class XsdManager {
	...
	public function __construct()
	{
		...
		switch($argc)
		{
			case 3: // new XsdManager(fileName, pageHandler, moduleHandler)
				if(is_string($argv[0]) 
				...
				break;
			case 4: // new XsdManager(fileName, pageHandler, moduleHandler, booleanDebug)
				if(is_string($argv[0]) 
					&& is_object($argv[1]) && get_class($argv[1]) == 'PageHandler'
					&& is_object($argv[2]) && get_class($argv[2]) == 'ModuleHandler'
					&& is_bool($argv[3]))
		...
		if($this->xsdFile==null)
		{
			...
			switch($argc)
			{
				...
				case 4:
					...
					if(is_object($argv[3])) $argv2 = get_class($argv[3]);
					else $argv2 = gettype($argv[3]);
