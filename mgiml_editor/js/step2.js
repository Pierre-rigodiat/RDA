...
addItem = function()
{
	...
	{
		var unavailableElement = $(this).parent().parent();

		while(!unavailableElement.hasClass('unavailable'))
		{
			console.log(unavailableElement);
			unavailableElement = unavailableElement.parent().parent();
		}
...
editionUnavailableBox = function(elementName, element)
{
	dhtmlx.modalbox({ 
		...
		callback:function(index){
			if(index==1)
			{
				element.find('.addItem').trigger('click');				
				dhtmlx.message(/*{
							type:"error",
...
registerAction = function(elementId, actionTag)
{
	...
	xmlhttp.open("GET","ajax/registerAction.php?id="+elementId+"&action="+actionTag, true);
	xmlhttp.send();
};