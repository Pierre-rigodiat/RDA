...
require_once 'inc/lib/XmlParserFunctions.php';
...
class XsdParser {
		...
		if($element['name']!=strtoupper($this->namespace).'COMPLEXTYPE' && $element['name']!=strtoupper($this->namespace).'SEQUENCE')
		{
			...
			else 
			{
				...
				{		
					...
					else echo $attr_name.': '.$this->elementList[$elementId]['attributes'][$attr_name];
						
					if(end($attr_name_array)!=$attr_name) echo ' | ';	
				}
				echo '</span>';

				echo ' <a href="#edition_popup" onclick="configurePopUp(\''.htmlspecialchars($attributes).'\')">Edit</a>'; // TODO Use the icon instead
	...
	private function organizeTree2($elementId)
	{
		...
		if($element['name']!=strtoupper($this->namespace).'COMPLEXTYPE' && $element['name']!=strtoupper($this->namespace).'SEQUENCE')
		{
			...
			{
				echo ' <input type="text" class="text" name="'.$this->path.'-0" id="'.$this->path.'-0" value="xxx"/>'; // XXX We need to specifie the id and name of this element;
				// XXX class attribute must be defined another way...
			}
	...
	private function displayXMLELement($elementId)
	{
		...
		for($i=0; $i < $repeat; $i++)
		{
			if(sizeof($element['children'])==0)
			{					
				$this->xmlString .= '<'.$element['attributes']['NAME'].'>'.$this->contentList[$keys_array[$i]].'</'.$element['attributes']['NAME'].'>';			
				//echo 'unset ' .	$keys_array[0] . '<br/>';	
				unset($this->contentList[$keys_array[$i]]);
				//echo $this->contentList[$keys_array[0]];
